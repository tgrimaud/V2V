...
- Setting `VOICE_BACKEND_URL` to the base host for the http voice backend — it must be the full `/api/conversation/converse` URL; the adapter derives the `converse-stream` / `warm-up` siblings from it, and a base host yields 404s on those.
