...
- Sending `tts_response.wav` from the batch `/api/voice/turn` handler — with backend streaming on (default) the answer arrives as one `TextFrame` per sentence, so `TtsFrameProcessor.response` is overwritten and holds only the **last** sentence; the caller then hears only the closing sentence (BUG-015). The full answer is in `BatchTurnResult.audio` (accumulated by `_AudioCaptureSink`): wrap that once as WAV and use it for both the response body and the egress byte count. WebRTC is unaffected (each sentence frame goes straight to the transport track). Quick mitigation without a rebuild: `VOICE_BACKEND_STREAM=0`.
- Trusting the Ansible voice deploy's "Wait for voice HTTP health" gate — it probes `http://127.0.0.1:8090/`, which returns `000` on the pilot Podman hosts even when the bridge is `healthy` and answers `200` on the host LAN IP. With `serial:1 max_fail_percentage:0` this false-negative aborts the whole play **after** the node was already recreated at the new tag, so the next node is never touched. Verify health out-of-band via the host IP / `docker inspect …Health.Status`; finish a blocked node by bumping `IMAGE_TAG` in its `/opt/voice-support/voice/.env` (preserve the vault-rendered secrets — never hand-write `.env`) then `sudo bash -c 'cd …/voice && IMAGE_TAG=<tag> podman compose pull && podman compose up -d --remove-orphans'`. Follow-up: repoint `health_url` off loopback.
- Probing an Ansible deploy health gate over **host HTTP** for a tier whose published port is loopback-quirky or firewalld source-scoped (the voice bridge `:8090` — `curl http://127.0.0.1:8090/` returns `000`, management FQDN is TASK-OPS-004 source-scoped) — the `uri` probe false-fails and, with `serial:1 max_fail_percentage:0`, hangs ~15 min then aborts the whole play. When the container defines a `HEALTHCHECK`, poll the **container's own verdict** instead: `docker inspect --format {{.State.Health.Status}} <name>` until `healthy` (it runs the check *inside* the container namespace, immune to host interface/firewall issues). Gate it with `health_container_name` and keep the HTTP `uri` probe as a fallback for tiers without a healthcheck (TASK-INFRA-011). Escape the Go template with `{% raw %}{{.State.Health.Status}}{% endraw %}` or Ansible tries to template it.
- Trying to put `CREATE DATABASE` / `CREATE ROLE …LOGIN` / `CREATE EXTENSION vector` in the app-startup Liquibase changelog — impossible: Liquibase connects *into* a DB *as* the login role, so it can't create the DB it connects to (also non-transactional), can't create the role it logs in as, and `vector`/`uuid-ossp` are superuser-only (the pilot app role isn't superuser). Split it: app changelog (tables) at startup as app user; a **separate superuser bootstrap changelog** (extensions/grants) run once with its own tracking-table names; DB/role/`ALTER … OWNER` stay a psql pre-step (TASK-INFRA-009, ADR-0041).
- Hand-writing the Liquibase `vector_store` DDL from memory — it must match Spring AI's PgVectorStore **exactly** or RAG breaks. Extract the real DDL from the jar (`javap -c org/springframework/ai/vectorstore/pgvector/PgVectorStore.class`): `metadata` is **`json`** (not jsonb), `embedding vector(768)`, index `spring_ai_vector_index USING hnsw (embedding vector_cosine_ops)`, id `uuid DEFAULT uuid_generate_v4()`. Guard with `not tableExists`/`MARK_RAN` for legacy dev DBs, add a schema-parity test, and re-verify on any Spring AI upgrade.
- Assuming a new dependency only *adds* to the classpath — it can silently *downgrade* another dep's transitive. `liquibase-core` pulled `commons-io 2.16.1` and Maven mediation used it over what `commons-csv 1.14.1` needs (≥ 2.17) → `NoClassDefFoundError`. After adding any dep, re-run `mvn test` and check `mvn dependency:tree -Dverbose` for the artifacts your other libs rely on; pin the conflict in `dependencyManagement` (the Spring Boot 3.4.1 BOM does not manage `commons-io`).
- Disabling Spring AI `initialize-schema` without replacing the extension creation it silently did — `initialize-schema: true` ran `CREATE EXTENSION vector`/`hstore`/`uuid-ossp` (worked in dev only because the dev pgvector user is superuser). Once off, provision the extensions elsewhere: pilot via the superuser bootstrap changelog, local dev via a `/docker-entrypoint-initdb.d/` init script — otherwise the first `vector(768)` DDL / `uuid_generate_v4()` insert fails.
- Passing a Postgres superuser password as `--password=…` on a `podman run liquibase` command line — visible in `ps`/shell history on the VM. Use `LIQUIBASE_COMMAND_PASSWORD` (env) or a mounted `liquibase.properties` instead, and pin the liquibase image to a patch/digest, not a floating minor tag.
- Building voice↔backend cross-tier tracing by assuming a live OpenTelemetry context exists during the HTTP call — the voice runtime exports telemetry **post-hoc**, so there is none. Derive a **deterministic** `traceparent` from the `correlation_id` (pure BLAKE2b, `voice_common/trace_context.py`), inject it on the hop, and anchor the exported `voice.turn` root under the **same** derived context. Never mint a fresh random trace id per turn (it splits voice and backend into unrelated traces).
- Leaving OTLP-exported child spans unparented (`tracer.start_span(name)` with no `context=`) — each child becomes its own trace root. Capture `set_span_in_context(root)` and pass it to every child so a turn is one span tree.
- Changing the end-of-turn runtime hold by editing the detector library's `DEFAULT_SILENCE_WINDOW_MS` — the **streaming** default is resolved in `web_voice/webrtc_signaling.py::_silence_window_config()` (350 ms via `PILOT_END_OF_TURN_SILENCE_MS`); the library constant stays 500 ms for batch/fixture. Flip the runtime default there, and update the two tests that assert the unset-config value.
- Flipping a voice code default (e.g. `VOICE_BACKEND_STREAM`) without checking every call path — verify the guarded branch (`_stream_this_turn()` only streams when the backend exposes `answer_stream`) so stubs/fakes keep the safe path, and invert env parsing to fail-safe (`not in ("0","false","no","off")`) so a bad value doesn't silently disable the validated lever.
- Hardcoding the REST path into `VOICE_BACKEND_URL`: since BUG-013 it is the backend **server base** URL (e.g. `http://host:8080`); `backend_factory` appends `/api/conversation/converse` and the adapter derives the `converse-stream` / `warm-up` siblings. A legacy full converse URL is still accepted (idempotent), but new config should be the base only. Do NOT point it at the backend VIP `:8080` on the pilot — HAProxy binds the backend VIP on `:80`.
- Assuming an `ansible localhost -m template` QA render won't see shared deploy vars — it auto-loads the implicit `all` group from the cwd basedir, so a new variable belongs in `group_vars/all/vars.yml` (with a safe default like `otel_collector_endpoint: ""`); passing it only via `-e @group_vars/<tier>.yml` is not required for the render to succeed.
- Assuming the backend needs a new "vetted-only" streaming contract before the runtime can speak sentence-by-sentence (latency lever 1) — `POST /api/conversation/converse-stream` already vets **each sentence** before emitting it (`GuardedSentenceEmitter`: ground → per-sentence `OutputGuardrail` → stop + hand-off on block), so DEC-002 holds backend-side. Consume the existing stream; don't design a redundant contract. Verify the property the runtime relies on with a **service-level incremental-delivery** test (first vetted sentence delivered before the full answer completes).
- Writing a warm-up path that injects a `ConversationMemoryPort` or lets a cold-provider failure throw — a warm-up must be structurally side-effect-free (no memory dependency), discard its output, be repeatable, and **record a miss instead of throwing** (`recordLatency(slice, provider, "error", …)`), so a cold STT/LLM can never block or delay the first real turn. Expose it as an always-200 endpoint with per-model flags (`POST /api/conversation/warm-up`, api-key gated in `WebSecurityMvcConfig`).
- Passing `-r` to ripgrep expecting "recursive" — `rg -r`/`-rl` is the **replace** flag (`-rl "x"` = replace matches with `l`), which silently rewrites the printed content and makes real strings look absent/garbled. rg recurses by default; use `-l` for files-with-matches and never `-r` unless you intend a replacement.
- Forgetting the two-level branch model when a ticket branch is validated — ticket branches fork from the sprint branch and merge back into it with `git merge --no-ff` (explicit integration marker); the sprint branch merges to `feat/restart-from-scratch` only at sprint closure on the user's explicit request. Fork the next ticket off the **sprint** branch (not the previous ticket branch) so shared backlog/doc definitions are already present as its base.
...
- Trusting a subagent's "error/failed" status at face value and blindly redoing its work — inspect the actual git state first (`git log`, `git status -sb`, ahead/behind vs origin); the work may be done (even pushed), and a zombie turn may still be committing **concurrently** in the same working tree (observed: HEAD moving between two status checks).
- Assuming the branch you're committing to is the active sprint — verify it wasn't already **closed + released** (release tag + closure merge + `git merge-base` vs mainline). Layering work on a closed/released sprint branch silently diverges from the real mainline (sprint-12 was `v0.6.0`-released while mainline was already on `v0.7.0`).
