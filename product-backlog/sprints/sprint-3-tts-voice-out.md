...
answer (TASK-WEB-003), no streaming playback (TASK-WEB-004, Sprint 4), no barge-in
(US-021).
...
- **Transport: Gradium TTS WebSocket** (`wss://api.gradium.ai/api/speech/tts`) — the
  ...
  audio chunks until `end_of_stream`, then play once. Streaming playback is Sprint 4.
...
| TASK-WEB-004 (streaming TTS) | Incremental playback (time-to-first-audio lever) — **Sprint 4** (latency optimization). |
