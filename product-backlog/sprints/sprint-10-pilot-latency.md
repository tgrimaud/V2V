...
| TASK-WEB-021 | Lever 2 — connect-time warm-up of the STT session + first LLM/embedding call (mirror `TtsSessionWarmer`) | Optimize | **Runtime implemented 2026-07-29** — shared `SessionWarmer` pre-opens STT at connect (no leak, `VOICE_STT_PREWARM`) + non-blocking `backend.warm_up()` trigger (`POST /warm-up`, `VOICE_BACKEND_WARMUP`); unittest 439 / behave 33 green. Pending live turn-1 cold-vs-warm sample |
