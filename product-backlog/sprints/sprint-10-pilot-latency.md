...
| TASK-WEB-020 | Lever 1 — stream the backend answer to TTS on the first vetted sentence (consume `converse-stream` SSE instead of blocking `/converse`) | Optimize | **Implemented + warm live validated — GO to enable on pilot, code default OFF** — flag `VOICE_BACKEND_STREAM`; per-sentence DEC-002 confirmed (5/5 grounded); confidence = option A (advisory); barge-in aborts + closes socket; `backend.first_token` now = first sentence. Tests green (unittest 462, behave 13·36·169). **Live 2026-07-30 (warm, n=5, same warm backend both runs):** `backend_first_token` p50 −658 ms, **mouth-to-ear p50 2696.9 → 1830.6 ms (−866)** / p95 4848.7 → 2526.1; fillers 4→1; barge-in 4/4. **ADR-0029 gate still FAIL** (m2e p95 2526 > 1500) — biggest single mover, needs STT finalize-tail + lever-2 to close. Evidence: `streaming-voice-qa-report.md` "Live Lever-1 Pass" |
