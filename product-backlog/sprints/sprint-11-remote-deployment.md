...
**TASK-INFRA-001** and **TASK-BE-022** (all adversarial + QA passed); integrated `mvn test`
**336** green, ArchUnit OK; compose stacks `qa-validate.sh` 22/22 green. Remaining:
TASK-INFRA-002/003, TASK-OPS-001/002, TASK-DOC-003. Scope locked with
...
| TASK-OPS-001 | GitHub Actions CI: `mvn test` + voice-agent `unittest`/`behave`, build & push both images to the registry, version/tag scheme | CI | 🚧 Implemented (2026-08-04) on `task/TASK-OPS-001-github-actions-ci` — `ci.yml` test gate + `images.yml` build/push to GHCR; `actionlint` clean; pending adversarial review + QA |
| TASK-OPS-002 | Ansible deploy playbooks + release/rollback runbook (`docs/operations/release-process.md`): render `.env` from secrets, `docker compose up`, voice **session draining**, tag-based rollback | Release | To do |
