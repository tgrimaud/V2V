...
| TASK-OPS-001 | GitHub Actions CI: `mvn test` + voice-agent `unittest`/`behave`, build & push both images to the registry, version/tag scheme | CI | 🚧 Implemented (2026-08-04) on `task/TASK-OPS-001-github-actions-ci` — `ci.yml` test gate + `images.yml` build/push to GHCR (reusable `tests.yml` gates both); `actionlint` clean; ✅ adversarial 93/100 (Pass) — blocking test-gating fix applied; ✅ QA GO (22/22 checks, [report](../../docs/qa/task-ops-001-github-actions-ci-qa-report.md)); ✅ merge-ready |
