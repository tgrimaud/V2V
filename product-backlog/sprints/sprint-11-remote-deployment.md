...
`qa-validate-haproxy.sh` 25/25 green (incl. real `haproxy -c`). Remaining:
TASK-INFRA-003, TASK-DOC-003. Scope locked with
...
| TASK-OPS-003 | Ansible host prerequisites: install Docker Engine + compose v2 plugin (Rocky EL9) + tier-aware firewalld, before deploy (the compose_tier role assumes Docker exists) | Provision | To do (bare VMs need this first) |
