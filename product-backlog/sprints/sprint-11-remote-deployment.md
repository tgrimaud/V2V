...
| TASK-INFRA-001 | docker-compose deploy stacks + `.env` templates per tier (backend→Postgres `.102`/Redis `.107`/embeddings/Mistral; voice→backend VIP `.11`/Gradium; Redis stack) | Wire | 🚧 Implemented (2026-08-04) on `task/TASK-INFRA-001-compose-stacks` — 3 stacks + `.env.example` + README, all pass `docker compose config`; pending adversarial review + QA |
