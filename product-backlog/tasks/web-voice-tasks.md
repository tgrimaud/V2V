...
**Status:** 🔧 In progress — **spike done + Slice 1 landed (2026-08-26).** The build is sliced to keep the suite green at every step.
...
- ✅ **Slice 1 — aiohttp HTTP surface (scope 2 partial, scope 3):** `web_voice/app.py::make_app(processor, signaling)` serves static + OpenAPI + all `/api/voice/*` REST (`turn`/`stt`/`tts`/`webrtc/offer`) with **byte-identical contracts** (204/404/411/413/502, JSON shapes, `application/json` no-charset, path-traversal guard, body cap). Blocking processor/signaling calls run in a thread executor so the single loop stays responsive. Selectable via `--server aiohttp` / `VOICE_SERVER=aiohttp` (default still `stdlib` this slice). Parity suite `tests/test_web_voice_app.py` (20 tests) + full suite **584 unit + 46 behave** green; live boot on one port confirmed (`server=aiohttp`, GET `/`, `/ws.html`, openapi, `/turn`, 404 all correct). The batch `/api/voice/turn` BUG-015 whole-answer WAV is preserved (reuses `_full_turn_response`).
- ⏳ **Slice 2 — WS upgrade onto the aiohttp port (scope 2 core, 4):** the hard part. Pipecat's `SingleClientWebsocketServerTransport` wraps the `websockets` library and **cannot share aiohttp's listener**; needs a pipecat input/output transport backed by aiohttp `WebSocketResponse` + the existing `WebSocketAudioSerializer`, feeding `SessionFactory.build_session` (ADR-0043). Then lift concurrency 1→N and retire `:8091` / `VOICE_WS_PORT`.
- ⏳ **Slice 3 — WebRTC re-home/guard (scope 5), edge confirm + compose/group_vars cleanup (scope 6), flip `--server` default to aiohttp, live ADR-0029 re-measure on the single routed port (scope 7 / TASK-WEB-039).**
