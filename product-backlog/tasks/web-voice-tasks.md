...
**Status:** ✅ **Merged into `feat/restart-from-scratch` (2026-08-26, `--no-ff`, released as `v0.7.0`)** — the edge-routing sub-scope was reverted (decision 2026-08-26, [ADR-0047](../../docs/architecture/adrs/ADR-0047-single-async-http-websocket-server-one-port.md)): rather than a `voice_ws` edge special-case, the runtime unifies HTTP+WS on one routed port (TASK-WEB-038) and HAProxy tunnels the upgrade on the existing `voice_bridges` backend. This ticket delivered the transport-agnostic bits (same-origin `ws.js` + WebRTC demotion); the HAProxy `voice_ws` ACL/backend + `firewall_extra_ports:[8091]` were removed. Live pilot confirmation of the routed `ws` turn is re-scored under **TASK-WEB-039**.
...
**Status:** ✅ **Merged into `feat/restart-from-scratch` (2026-08-26, `--no-ff`, released as `v0.7.0` + deployed to eir-ai4cc-tst).** spike + Slice 1 + Slice 2 + Slice 3 landed; the build was sliced to keep the suite green at every step. The aiohttp single-port server is now the **default** and the shipped runtime; the only residual is the on-pilot live confirmation (edge `101` + ADR-0029 re-measure), tracked under scope 7 / TASK-WEB-039.
