...
**Status:** ✅ Done — shipped to `feat/restart-from-scratch` via Sprint 13 closure (2026-08-31); AudioHook connection auth (API key + HMAC-SHA256 signature, fail-closed, golden-vector locked) + Architect flow control/routing contract. `@request-target`/`@authority` behind the edge resolved from the deployed HAProxy config (TASK-INFRA-016 — pass-through, no override); exact live-tenant header casing/secret + Architect wiring pending live Genesys measurement (OQ-006)
...
**Status:** ✅ Done — shipped to `feat/restart-from-scratch` via Sprint 13 closure (2026-08-31); genesys-ready image `v0.8.0` deployed to `vla-t01/t02` with `VOICE_GENESYS=on` (Step 0b self-test PASSED). The AudioHook shared secret is generated by us and stored in the local git-ignored `vault.yml` for the pre-live self-test (rotated to the admin-agreed secret before the live-org test).
...
## TASK-OPS-012 — Post–Sprint-13 alignment sweep (code + documentation)

**Type:** Operations — doc/ledger hygiene + deploy guard
**Priority:** Medium
**Depends on:** Sprint 13 closure + TASK-INFRA-016
**Classification:** documentation + ledger reconciliation; one runtime-adjacent deploy guard (Ansible assert). Not voice-runtime-affecting.
**Status:** ✅ Done (2026-09-04) on `task/TASK-OPS-012-post-sprint13-alignment` (off `feat/restart-from-scratch`).

### Context

Before resuming Genesys work, a full code + documentation review (three parallel `explore`
agents: connector-code-vs-docs, deployment-config, backlog/ledger/ADR) surfaced a systematic
**post-closure drift**: Sprint 13 closed + merged into `feat/restart-from-scratch` and a
genesys-ready image (`v0.8.0`, `VOICE_GENESYS=on`) was deployed, but many ledger rows/ticket
sections still read "merged into sprint-13 / prep / proposed / not merged / sprint in
progress", several ADR status blocks were stale, some ops docs still described Genesys as
future/not-part-of-the-pilot, a few docs mis-stated the wire codec/sample rate, and the
Ansible deploy had no guard for the Genesys secrets when the endpoint is enabled.

### What was implemented

- **Ledgers reconciled** to the merged/deployed reality: `backlog-index.md` (11 Sprint-13
  ticket rows → ✅ Done; sprint-registry WEB-044 → Open; stale "do not mint ADR-0049" note),
  full ticket sections in `web-voice-tasks.md` / `backend-hardening-tasks.md` /
  `deployment-tasks.md` (WEB-025/041/042/043/047, BE-036/037, INFRA-012/015), the sprint-13
  file (closure note + ticket table + branch section + exit criterion for WEB-044), and the
  `done-tasks.md` closure entry.
- **ADRs refreshed:** ADR-0040 (NOT IMPLEMENTED → partially implemented/deployed), ADR-0047
  (impl pending → shipped in v0.7.0), ADR-0049 (implementation note under Decision; §4
  control-mode default `detector`; §6 auth implemented), and the ADR `README.md` built-vs-target.
- **Ops docs updated:** `flow-requests-eir-ai4cc-tst.md` §3 (future/do-not-bundle → active
  inbound-`:443`-from-Genesys request + private-CA blocker), `genesys-live-measurement-runbook.md`
  Step 1 (endpoint deployed, resolved URL, auth implemented, TLS blocker),
  `deployment-eir-ai4cc-tst.md` (added the Genesys env subsection), the admin doc rows
  (path/@authority resolved), the architect-flow-contract To-Confirm rows, and `CLAUDE.md`.
- **Accuracy:** wire codec corrected to **8 kHz L16/PCMU (internal 16 kHz)** in the admin doc,
  `.env.example`, and the runbook; stale HMAC-deferral comment in `genesys_config.py` fixed.
- **Deploy guard:** `deploy.yml` now asserts `vault_genesys_audiohook_api_key`/`_secret` are
  set (not `CHANGE_ME`) when `voice_genesys` is `on`/`auto`, and `qa-validate-ansible.sh`
  checks the two Genesys secret keys are sourced from vault vars.

### Acceptance (met)

- Tests green: voice-agent **713 unit + 18/51/225 behave**, backend **`mvn test`** OK.
- `deploy.yml` passes `ansible-playbook --syntax-check`; `qa-validate-ansible.sh` `bash -n` clean.
- No secret values committed; ledger statuses no longer contradict the merged/deployed state.

### Out Of Scope

- No runtime/behaviour change to the voice or backend code (docs + Ansible guard only).
- The live-org measurement campaign and the private-CA/network-path blocker resolution
  (tracked in OQ-006 + the admin doc).
