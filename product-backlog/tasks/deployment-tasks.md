...
**Classification:** V1 pilot deployment (release correctness)
**Status:** 📋 Planned — recurring blocker. **Registered on the sprint-12 branch 2026-08-28**
from the deploy/observability adversarial review (Major M1,
`docs/qa/adversarial-review-2026-08-28-deploy-observability.md`), so the fix is tracked and not
tribal knowledge.
**Priority:** High
**Branch:** `task/TASK-INFRA-011-voice-health-gate-loopback` (to create)

> **ID note (cross-branch reuse, not re-mint).** This id is **already allocated on mainline**
> (`feat/restart-from-scratch`) and on `feat/sprint-13-genesys-audio-connector` for the exact same
> issue (dedicated branch `task/TASK-INFRA-011-voice-health-container-probe`). This sprint-12 branch
> only forward-referenced `TASK-INFRA-011` in prose; the deploy M1 finding is closed by **registering
> the existing ticket here**, not by minting a duplicate `TASK-OPS`/`TASK-INFRA` number. Reconcile at
> Sprint 12 closure.

### Context

The Ansible voice deploy's "Wait for voice HTTP health" step probes `http://127.0.0.1:8090/`
(`group_vars/voice.yml` `health_url`, consumed by `roles/compose_tier/tasks/health.yml`,
`retries: 30 delay: 5`). On the pilot podman hosts the published port answers on `0.0.0.0:8090`
and via the **host LAN IP** (`curl http://<host-ip>:8090/` → `200`, container `healthy`), but
**host→loopback** returns `000` — a podman port-forwarder quirk, not an app failure.

With `serial: 1` + `max_fail_percentage: 0` (`deploy.yml`), the stack is recreated (`compose pull`
+ `up -d`) **before** this gate, so the false negative burns ~150 s then aborts the whole voice
play **after** the node was already recreated at the new tag — the second bridge is never touched
and the fleet is left on mixed tags.

### Scope

- Replace the loopback HTTP probe with one of:
  - **(preferred)** the container's own health verdict —
    `podman inspect --format '{{.State.Health.Status}}'` (the image already ships a `HEALTHCHECK`),
    which is immune to the host→loopback forwarder quirk; or
  - repoint `health_url` at the host's primary LAN IP (`ansible_default_ipv4.address`) instead of
    `127.0.0.1`; or
  - make the gate non-fatal (`failed_when: false`) with an explicit out-of-band confirmation task.
- Keep the gate meaningful (still fail on a genuinely unhealthy container) and keep `serial: 1`
  safe (don't recreate the next node on a real failure).
- Sweep the same loopback assumption on the backend tier probe (`backend.yml` `/actuator/health`)
  for consistency.
...
- `qa-validate-ansible.sh` stays green; add a check asserting the gate is not a bare `127.0.0.1`
  probe.
- Documented in the deploy runbook (the manual "finish a blocked node" workaround can be retired).

### Notes

- Interim manual workaround (already documented in `CLAUDE.md`): verify health out-of-band
  (`docker inspect --format {{.State.Health.Status}}` + `curl http://<host-ip>:8090/`), then finish
  a blocked node by bumping `IMAGE_TAG` in `/opt/voice-support/voice/.env` (preserve the
  vault-rendered secrets — never hand-write `.env`) and
