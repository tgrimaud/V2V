...
**Status:** 🔧 In progress (2026-08-27) — benchmark harness + billing fixture set + ADR-0045 comparison merger landed on `task/TASK-BE-033-llm-provider-benchmark` (`scripts/llm_benchmark/`); QA evidence scaffold at `docs/qa/task-be-033-llm-provider-benchmark-evidence.md`. All four candidates are now measurable behind the port: candidates 1–3 (Mistral small/large, co-located Ollama) via `LLM_PROVIDER` + model env vars, and candidate 4 (`openai-gpt-4o-mini`) via the new `OpenAiAnswerAdapter` + `spring-ai-starter-model-openai` dependency (user-approved 2026-08-27) wired in `LlmConfig` (`LLM_PROVIDER=openai` + `OPENAI_API_KEY`); backend `mvn test` green (401 tests, incl. `OpenAiAnswerAdapterTest`), `commons-io` unchanged at 2.19.0. OpenAI is measurable for the spike only — selecting it for the pilot still needs the `api.openai.com:443` egress allowlist entry (ADR-0039) + OQ-009 US-egress sign-off. **EU/on-prem subset measured (2026-08-27, local):** server `backend_first_token` p50/p95 ms — `mistral-small` **844/3039** (grounded 1.0, 0 err, 0 €-flags) = best; `mistral-large` 8789/24108 with **9/15 errors** (API-tier throttling) + 20× cost = **disqualified**; `ollama llama3.1:8b` raw chat token p50 **338 ms** but `backend_first_token` 2505/8823 **inflated by single-instance embed↔chat swap contention** → needs dedicated-capacity re-measure. Evidence: `docs/qa/task-be-033-llm-provider-benchmark-evidence.md` + `task-be-033-comparison-2026-08-27.json`. **Recommendation: keep `mistral-small` for pilot; `ollama` = sovereignty fallback to re-measure; OpenAI unmeasured (no key/OQ-009).** Also fixed: exclude the full OpenAI auto-config set (audio-speech/transcription/image) so a keyless boot doesn't crash. **OpenAI re-test kept one-command-ready** (no code change): `scripts/llm_benchmark/run_local_candidate.sh` (portable, JDK auto-detect) + `OPENAI_API_KEY` → re-merge into the 4-row table; ADR-0045 deliberately stays **Proposed** so the OpenAI row can complete before the EU-vs-US decision. Surfaced by WEB-036 (LLM first-token ~87% of `backend_first_token`) + WEB-035 re-score (backend first-token p95 ~1199 ms is the largest remaining reducible slice of the ADR-0029 gate).
**Priority:** High
