...
| TASK-DEPLOY-002 | Voice bridge Python Docker image (heavy deps `pipecat-ai`/`aiortc`/`opencv-python`, `--host 0.0.0.0`, healthcheck `/`, non-root) | V1 pilot deployment | 🚧 Implemented (2026-08-03) on `task/TASK-DEPLOY-002-voice-image` — `voice-agent/Dockerfile` + `.dockerignore`, image build + runtime smoke validated (imports pipecat/cv2/aiortc, binds `0.0.0.0:8090`, `GET /` 200, non-root uid 1001); pending adversarial review ≥90% + QA. EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
