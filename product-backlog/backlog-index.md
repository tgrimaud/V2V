...
| TASK-BE-022 | Constant-time API-key gate unification (`ApiKeyGuard` across `/converse` + `/converse-stream`) + client-controlled log/header sanitization (`correlation_id`/`channel`) — findings #1 & #3 of the 2026-08-04 backend adversarial review | V1 security hardening | ✅ **Merge-ready** (2026-08-04, adversarial **95/100** + QA **GO**) on `task/TASK-BE-022-auth-log-hardening` (from `feat/restart-from-scratch`) — both converse controllers now delegate to `ApiKeyGuard.authorized` (`MessageDigest.isEqual`, dropping the `String.equals` timing side-channel + the 3× gate duplication); new `CorrelationId.sanitize` strips ISO control chars + caps 200 chars on every client id/channel before MDC/log/response header. `mvn test` **336** green (+5 `CorrelationIdTest`, +1 MVC CR/LF-echo regression), ArchUnit OK. QA: `docs/qa/task-be-022-auth-log-hardening-qa-report.md`. Merge on explicit user request only — `tasks/backend-hardening-tasks.md` | Medium |
| TASK-BE-023 | Restrict the unauthenticated ops surface (`/swagger-ui`, `/v3/api-docs`, `/actuator/metrics`) before any non-localhost exposure — finding #4 of the 2026-08-04 backend adversarial review | V1 security hardening | Proposed (2026-08-04) — **ticket only, deferred**. Acceptable on the localhost pilot (BE-019 kept the docs/metrics open intentionally); blocking before the backend answers off-box (Sprint 11 remote deployment). Mechanism to design (x-api-key gate on docs, Actuator split/management port, or env-driven exposure like the BE-021 `REDIS_HEALTH_ENABLED` precedent); keep `/actuator/health` open for probes — `tasks/backend-hardening-tasks.md` | Medium |
