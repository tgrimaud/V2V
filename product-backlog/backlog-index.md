...
| TASK-DEPLOY-001 | Backend Java Docker image (multi-stage JDK17→JRE17, non-root, `HEALTHCHECK /actuator/health`, fully env-driven; add/pin Maven in build stage — no `mvnw` today) | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-DEPLOY-001-backend-image`). EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
| TASK-DEPLOY-002 | Voice bridge Python Docker image (heavy deps `pipecat-ai`/`aiortc`/`opencv-python`, `--host 0.0.0.0`, healthcheck `/`, non-root) | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-DEPLOY-002-voice-image`). EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
| TASK-BE-021 | Redis-backed conversation memory (`RedisConversationMemoryAdapter`, `CONVERSATION_STORE=redis`) so the 2 backends behind VIP `.11` share session state — activates ADR-0008; JUnit manual fakes, bean wired by profile/flag | V1 pilot deployment | ✅ Merge-ready (2026-08-03, adversarial 92/100 → QA GO) on `task/TASK-BE-021-redis-conversation-memory` — `RedisConversationMemoryAdapter` (list+TTL) behind a `ConversationTurnStore` seam, `spring-boot-starter-data-redis`, wired by `CONVERSATION_STORE`; degrades on outage. Blocking fix: Actuator Redis health indicator gated `REDIS_HEALTH_ENABLED` (default off) → `/actuator/health` stays UP in memory mode (live-verified). `mvn test` 330 green (+10, ArchUnit OK), default `memory` unchanged. EPIC-012 / ADR-0008 / ADR-0038 — `tasks/deployment-tasks.md`, `docs/qa/sprint-11-deployment-qa-report.md` | High |
| TASK-INFRA-001 | docker-compose deploy stacks + `.env` templates per tier (backend→Postgres `.102`/Redis `.107`/embeddings/Mistral; voice→backend VIP `.11`/Gradium; Redis stack) | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-INFRA-001-compose-stacks`). EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
| TASK-INFRA-002 | HAProxy + Keepalived config for the two VIPs (voice `.10`→t01/t02 TLS edge, backend `.11`→t03/t04), health checks, finalized ports, VRRP failover — coordinated with the platform team | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-INFRA-002-haproxy-vips`). EPIC-012 / ADR-0038 / ADR-0033 — `tasks/deployment-tasks.md` | High |
| TASK-INFRA-003 | Decision + spike: embeddings placement (Ollama CPU co-located vs Mistral embeddings → 1024-dim `vector_store` recreation) and provider egress (Mistral/Gradium/registry) → ADR addendum | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-INFRA-003-embeddings-egress`); gated by open infra inputs. EPIC-012 / ADR-0006 / ADR-0038 — `tasks/deployment-tasks.md` | High |
| TASK-OPS-001 | GitHub Actions CI: `mvn test` + voice-agent `unittest`/`behave`, build & push both images to the registry, version/tag scheme (release tags) | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-OPS-001-github-actions-ci`). EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
| TASK-OPS-002 | Ansible deploy playbooks + release/rollback runbook (`docs/operations/release-process.md`): render `.env` from secrets, `docker compose up`, voice **session draining**, tag-based rollback | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-OPS-002-ansible-release`). EPIC-012 / ADR-0038 / ADR-0010 — `tasks/deployment-tasks.md` | High |
| TASK-DOC-003 | Keep deployment docs in sync as tickets land + author the operational first-deploy runbook (checklist, DB `CREATE EXTENSION vector`, smoke test) | V1 pilot deployment | To do (Sprint 11, branch `task/TASK-DOC-003-deployment-docs`). EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | Medium |
...
| SPRINT-11-REMOTE-DEPLOYMENT | Remote deployment & release readiness (eir-ai4cc-tst) (EPIC-012) | 🚧 In progress (started 2026-08-03; billing/identity deferred → Sprint 12, telephony/Genesys → Sprint 13) | Package the two-service stack (Docker + docker-compose), deploy to eir-ai4cc-tst (HAProxy/Keepalived VIPs, Redis-backed shared memory), and stand up CI/CD (GitHub Actions + Ansible/SSH) with a release/rollback runbook — `sprints/sprint-11-remote-deployment.md` |
| SPRINT-12-BILLING-IDENTITY | Customer identity + BSS/PDF evidence + deterministic comparison (EPIC-002/003/004) | Planned (tentative) — gated by OQ-001/003/004 (shifted from Sprint 11 on 2026-08-03) | Prove billing value with real customer identity and BSS/PDF evidence behind the answer engine |
| SPRINT-13-TELEPHONY-GENESYS | Telephony channel (US-018) + Genesys advisor handoff (EPIC-007) | Planned (tentative) — gated by OQ-006 (shifted from Sprint 12 on 2026-08-03) | Expose the journey over telephony and hand off to a Genesys advisor |
