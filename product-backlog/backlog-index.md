...
| TASK-BE-027 | Retrieval-quality eval harness + baseline (offline) — labeled FR/EN eval set with phrasing variants against `/api/conversation/retrieve`, computing recall@k / MRR / phrasing-stability; the OQ-008 / ADR-0032 measurement gate | V1 quality (RAG measurement) | 🚧 Implemented (2026-08-13, branch `task/TASK-BE-027-retrieval-quality-eval-harness`) — `scripts/retrieval_eval/` (9 metric tests), **baseline recorded**: overall recall@8 0.90 / stability 0.79; FR+billing+commercial pass, EN+support lag (3 phrasing flips). Awaiting adversarial review + QA. EPIC-005 / ADR-0032 / OQ-008 — `tasks/kb-ingestion-tasks.md` | Medium |
| TASK-BE-028 | Retrieval lever 2b — MMR (diversity) over the over-fetched candidates so near-duplicate header chunks stop evicting the answer chunk | V1 quality (RAG) | Proposed — **gate satisfied** by the TASK-BE-027 baseline (stability 0.79 < 0.90, support/EN flips); cheap, pgvector-native. EPIC-005 / ADR-0032 — `tasks/kb-ingestion-tasks.md` | Low |
