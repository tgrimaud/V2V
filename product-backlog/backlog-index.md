...
| TASK-BE-021 | Redis-backed conversation memory (`RedisConversationMemoryAdapter`, `CONVERSATION_STORE=redis`) so the 2 backends behind VIP `.11` share session state — activates ADR-0008; JUnit manual fakes, bean wired by profile/flag | V1 pilot deployment | 🚧 Implemented (2026-08-03) on `task/TASK-BE-021-redis-conversation-memory` — `RedisConversationMemoryAdapter` (list+TTL) behind a `ConversationTurnStore` seam, `spring-boot-starter-data-redis`, wired by `CONVERSATION_STORE`; degrades on outage. `mvn test` 327 green (+7, ArchUnit OK), default `memory` unchanged; pending adversarial review ≥90% + QA. EPIC-012 / ADR-0008 / ADR-0038 — `tasks/deployment-tasks.md` | High |
