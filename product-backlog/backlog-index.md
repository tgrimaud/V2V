...
| TASK-BE-019 | Authenticate/isolate the unauthenticated backend endpoints (`/api/knowledge/ingest`, `/sync`, `/api/conversation/answer`, `/retrieve`) — same `x-api-key` gate as `/converse` or internal-only binding; fail-closed + security tests | V1 security hardening | Proposed (2026-07-28) — from the full adversarial review (blocking security finding); **doable now** — `tasks/backend-hardening-tasks.md` | High |
| TASK-WEB-018 | Speak a degraded fallback on streaming STT failure (WebRTC) — parity with the batch `/turn` 502; no silent call, no fabricated transcript (DEC-002) | V1 hardening (degraded mode) | 🚧 Implemented (2026-07-28) on `task/TASK-WEB-018-streaming-stt-degraded-fallback` — `StreamingSttProcessor` speaks `DEGRADED_FALLBACK_TEXT` (plain TextFrame, no transcript) on finalize error/timeout + `voice.stt.degraded_spoken` event; unittest 391 + behave 31 scenarios green. Pending adversarial review + QA — `tasks/web-voice-tasks.md` | Medium |
