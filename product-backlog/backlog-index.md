...
| TASK-DEPLOY-001 | Backend Java Docker image (multi-stage JDK17→JRE17, non-root, `HEALTHCHECK /actuator/health`, fully env-driven; add/pin Maven in build stage — no `mvnw` today) | V1 pilot deployment | 🚧 Implemented (2026-08-03) on `task/TASK-DEPLOY-001-backend-image` — multi-stage `backend/Dockerfile` + `.dockerignore`, image build-validated (non-root uid 1001, Java 17, `EXPOSE 8080`, `HEALTHCHECK /actuator/health`, Spring Boot boots); pending adversarial review ≥90% + QA. EPIC-012 / ADR-0038 — `tasks/deployment-tasks.md` | High |
