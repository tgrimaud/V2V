...
| TASK-WEB-021 | Lever 2 — connect-time warm-up of the STT streaming session + first LLM/embedding call (mirror `TtsSessionWarmer`) to remove the turn-1 cold-start penalty (~−450 ms) | V1 pilot gate (perceived latency) | In progress (Sprint 10, branch `task/TASK-WEB-021-connect-time-warmup`): runtime **implemented** 2026-07-29 — shared `SessionWarmer` pre-opens the first STT session at `StartFrame` (no leak; `VOICE_STT_PREWARM=0` disables) + `AnswerProcessor` fires a non-blocking `backend.warm_up()` (`POST /warm-up`, TASK-BE-017) once at connect (`VOICE_BACKEND_WARMUP=0` disables); telemetry `voice.backend.warmup` (success/miss). unittest **439** / behave **33** green. Pending live cold-vs-warm turn-1 sample. ADR-0037 | High |
