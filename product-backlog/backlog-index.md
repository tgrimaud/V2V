...
| TASK-OBS-001 | OpenTelemetry export (OTLP) for backend + voice runtime (currently Micrometer + stderr spans), or record the accepted residual risk in ADR-0028 | V1 hardening (observability) | 🚧 Implemented — **hybrid** (2026-07-29) on `task/TASK-OBS-001-otel-export`: ADR-0028 addendum accepts the pilot residual risk + mandatory-export trigger, AND env-gated OTLP wiring (default OFF) on both services + opt-in collector recipe (`deploy/observability/`). Backend `mvn test` 312 green + export-off startup clean; voice unittest 396 (+5) + behave 31 green; live backend→collector smoke exported real traces + metrics. Cross-service correlated-trace (traceparent) deferred behind the trigger. Pending adversarial review + QA — `tasks/observability-tasks.md` | Medium |
