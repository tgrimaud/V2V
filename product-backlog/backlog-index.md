...
| TASK-WEB-020 | Lever 1 — stream the backend answer to TTS on the **first vetted sentence** (consume the `converse-stream` SSE guarded-sentence stream instead of the blocking `/converse`), targeting the largest slice (backend_first_token ≈ 1 s p50) | V1 pilot gate (perceived latency) | To do (Sprint 10, split from TASK-WEB-015): backend stream is **already DEC-002-safe per sentence** (`GuardedSentenceEmitter`), so no backend contract change needed for safety; feature-flagged default-off; depends on TASK-BE-017 (optional early confidence) + TASK-WEB-014 baseline. ADR-0037 | High |
