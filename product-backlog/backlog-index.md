...
| TASK-BE-027 | Retrieval-quality eval harness + baseline (offline) — labeled FR/EN eval set with phrasing variants against `/api/conversation/retrieve`, computing recall@k / MRR / phrasing-stability; the OQ-008 / ADR-0032 measurement gate | V1 quality (RAG measurement) | Proposed (branch `task/TASK-BE-027-retrieval-quality-eval-harness`). Offline, no pilot access. Baseline = fixed chunker + top-K=8 + dense-only. EPIC-005 / ADR-0032 / OQ-008 — `tasks/kb-ingestion-tasks.md` | Medium |
| TASK-BE-028 | Retrieval lever 2b — MMR (diversity) over the over-fetched candidates so near-duplicate header chunks stop evicting the answer chunk | V1 quality (RAG) | Proposed — **gated** on the TASK-BE-027 baseline showing eviction persists; cheap, pgvector-native. EPIC-005 / ADR-0032 — `tasks/kb-ingestion-tasks.md` | Low |
