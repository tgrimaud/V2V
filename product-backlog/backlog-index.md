...
| TASK-WEB-014 | Instrument true mouth-to-ear latency — fold `channel_egress` (WebRTC) + end-of-turn hold into a perceived-latency metric and evaluate vs ADR-0029 (closes the ADR-0018/TASK-WEB-009 known gap) | V1 pilot gate | Implemented (2026-07-23) — `voice_to_first_audio` composite + `ChannelEgressProbe` (WebRTC egress) + ADR-0029 gate + client first-audible proxy; unittest 334 / behave 26 green; docs updated. Remaining: warm live sample vs real backend + adversarial/QA. Branch `task/TASK-WEB-014-mouth-to-ear-latency` | High |
