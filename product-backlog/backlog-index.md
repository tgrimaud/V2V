...
| TASK-WEB-013 | Unify telemetry imports in `web_voice` — point `ingress.py` at `voice_common.telemetry` for symmetry with `egress.py` (closes RF-023) | V1 hardening | Implemented (2026-07-23, Sprint 9) — one-line import change, 334 unittest + 26 behave green; branch `task/TASK-WEB-013-unify-telemetry-imports`. Not runtime-affecting (same classes via shim) | Low |
