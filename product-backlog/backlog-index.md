...
| TASK-WEB-021 | Lever 2 — connect-time warm-up of the STT streaming session + first LLM/embedding call (mirror `TtsSessionWarmer`) to remove the turn-1 cold-start penalty (~−450 ms) | V1 pilot gate (perceived latency) | To do (Sprint 10, split from TASK-WEB-015): STT warm-up in the runtime + backend LLM/embedding warm trigger (TASK-BE-017); no leaked session, non-blocking failure; env-tunable. ADR-0037 | High |
