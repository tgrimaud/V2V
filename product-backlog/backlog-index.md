...
| TASK-WEB-014 | Instrument true mouth-to-ear latency — fold `channel_egress` (WebRTC) + end-of-turn hold into a perceived-latency metric and evaluate vs ADR-0029 (closes the ADR-0018/TASK-WEB-009 known gap) | V1 pilot gate | Proposed (2026-07-20, from ADR-0029) — out-of-sprint pilot-readiness follow-up (EPIC-010 / US-040), off the billing theme (now Sprint 9); schedule before the pilot | High |
| TASK-WEB-015 | Perceived-latency optimization levers — backend-stream-to-TTS (first sentence, ~−700-900 ms), connect-time STT/LLM warm-up (~−450 ms turn 1), end-of-turn hold tuning (~−150 ms); baseline measured at the Sprint 7 demo (cold ~2.95 s / warm ~2.50 s to first audio) | V1 pilot gate | Proposed (2026-07-20, from Sprint 7 demo) — out-of-sprint pilot-readiness, off the billing theme (now Sprint 9); depends on TASK-WEB-014 (optimize against a measured baseline) | High |
...
| TASK-BE-013 | `CsvArticleConnector` + embedding `DomainClassifier` — bulk KB ingestion from `articles.csv` (CommonsCSV, jsoup HTML→text, `language=en`, domain classified vs anchors) | V1 core (KB content) | Planned (Sprint 8) — was TASK-BE-011 (ID freed for the Sprint 7 latency ticket) | High |
...
| TASK-BE-018 | Concise voice-first answers — cap answer length via LLM prompt budget to cut TTS synthesis time (batch `/turn` ≈14 s, live tail); no grounding regression (BUG-004) | V1 answer quality / latency | Proposed (2026-07-23) — out of Sprint 8 theme; latency follow-up, ADR-0033/ADR-0029 | High |
...
| SPRINT-8-CSV-KB-INGESTION | CSV knowledge-base ingestion (articles.csv) | Planned (opened 2026-07-21) | Ingest the real operator KB CSV export (`articles.csv`, HTML `content`) via a new `CsvArticleConnector` on the BE-003 socle, with an embedding `DomainClassifier` to tag mixed content by domain (TASK-BE-013) and batched embedding/insert for the volume (TASK-BE-014). BE-013/BE-014 validated + merged 2026-07-21; TASK-BE-015 (answer language: reply in the customer's FR/EN language, EN default for Eir) scoped in and open. Billing/identity theme shifts to Sprint 9, telephony/Genesys to Sprint 10 — `sprints/sprint-8-csv-kb-ingestion.md` |
