...
| TASK-BE-028 | Retrieval lever 2b — MMR (diversity) over the over-fetched candidates so near-duplicate header chunks stop evicting the answer chunk | V1 quality (RAG) | 🚧 Implemented (2026-08-13, `task/TASK-BE-028-mmr-diversity`) — pure `MmrReranker` behind `VectorSearchPort`, env-tunable λ/over-fetch, `RetrievalObserverPort` observability, 8 new tests (347 backend green); eval re-run + adversarial + QA pending backend access. 2 EN misses split out to BUG-009. EPIC-005 / ADR-0032 — `tasks/kb-ingestion-tasks.md` | Low |
