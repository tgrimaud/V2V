...
| TASK-BE-022 | Constant-time API-key gate unification (`ApiKeyGuard` across `/converse` + `/converse-stream`) + client-controlled log/header sanitization (`correlation_id`/`channel`) — findings #1 & #3 of the 2026-08-04 backend adversarial review | V1 security hardening | 🚧 Implemented (2026-08-04) on `task/TASK-BE-022-auth-log-hardening` (from `feat/restart-from-scratch`) — both converse controllers now delegate to `ApiKeyGuard.authorized` (`MessageDigest.isEqual`, dropping the `String.equals` timing side-channel + the 3× gate duplication); new `CorrelationId.sanitize` strips ISO control chars + caps 200 chars on every client id/channel before MDC/log/response header. `mvn test` **335** green (+5 `CorrelationIdTest`), ArchUnit OK. Adversarial review + QA pending; merge on user request only — `tasks/backend-hardening-tasks.md` | Medium |
