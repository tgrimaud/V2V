...
| TASK-BE-028 | Retrieval lever 2b — MMR (diversity) over the over-fetched candidates so near-duplicate header chunks stop evicting the answer chunk | V1 quality (RAG) | Proposed — **gate narrowly satisfied**: baseline shows **1 retrieval eviction**; the 2 EN misses are guardrail `OFF_TOPIC` over-blocks (out of scope). Cheap, pgvector-native. EPIC-005 / ADR-0032 — `tasks/kb-ingestion-tasks.md` | Low |
