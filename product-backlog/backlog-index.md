...
| TASK-WEB-010 | End the call on a customer closing formula (US-041) — detect closing intent on final transcript, speak a closing, end the session; false-positive guard + end-of-call reason telemetry | V1 core | Implemented (2026-07-27, Sprint 9, ADR-0035) — pending adversarial review + QA + user validation | Medium |
...
| TASK-BE-016 | OpenAPI/Swagger for the Java backend — `springdoc-openapi` + `/swagger-ui` + `/v3/api-docs`, `@Tag`/`@Operation` on controllers | V1 hardening | Implemented (2026-07-28, Sprint 9) — springdoc 2.8.14 pinned, live OpenAPI 3.1; `mvn test` 305 green; pending adversarial review + QA + user validation | Medium |
...
| TASK-BE-019 | Authenticate/isolate the unauthenticated backend endpoints (`/api/knowledge/ingest`, `/sync`, `/api/conversation/answer`, `/retrieve`) — same `x-api-key` gate as `/converse` or internal-only binding; fail-closed + security tests | V1 security hardening | 🚧 Implemented (2026-07-28) on `task/TASK-BE-019-endpoint-auth` — central `ApiKeyAuthInterceptor` gates the 4 endpoints, sanitized 401 `ERR_401`, pilot semantics kept (empty key = open, enforced when set); `mvn test` **312** green (+7). Pending adversarial review + QA — `tasks/backend-hardening-tasks.md` | High |
| TASK-WEB-018 | Speak a degraded fallback on streaming STT failure (WebRTC) — parity with the batch `/turn` 502; no silent call, no fabricated transcript (DEC-002) | V1 hardening (degraded mode) | Proposed (2026-07-28) — from the full adversarial review; **doable now** — `tasks/web-voice-tasks.md` | Medium |
| TASK-OBS-001 | OpenTelemetry export (OTLP) for backend + voice runtime (currently Micrometer + stderr spans), or record the accepted residual risk in ADR-0028 | V1 hardening (observability) | Proposed (2026-07-28) — from the full adversarial review; **partially doable now** (ADR decision immediate; full OTLP needs a collector) — `tasks/observability-tasks.md` | Medium |
