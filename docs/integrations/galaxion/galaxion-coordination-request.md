...
- **Amounts are in integer cents** (confirmed 2026-09-09) → maps directly to our
  integer-minor-unit convention.
- **`crud_amount` is a technical field** (builds the VAT table), *not* a
  customer-facing amount (confirmed 2026-09-09) → **excluded** from the comparison.
...
### P2 — Amount semantics (mostly resolved 2026-09-09)

- ~~**Unit:** euros or cents?~~ **Resolved: integer cents.**
- ~~**`crud_amount` meaning?**~~ **Resolved: technical field for the VAT table — not
  a customer-facing amount; excluded from the comparison.**
- *Still open:* how are **taxes rounded**, and does the invoice total include
  **previous balance / payments** (`balance_previous_bc`, `overdue_amount`) or only
...
| Tax rounding + previous-balance/payment inclusion in total (P2; unit + `crud_amount` resolved) | Galaxion billing owner |
...
> follow-up. The amount questions are now resolved (thanks!) — this focuses on the
> remaining inputs to validate V1 on real data.

**Subject:** Galaxion Billing V1 — remaining inputs to validate on real invoices

Hi [name],

Thanks for the billing data model and the answers on amounts — that settles it:
amounts are **integer cents**, `crud_amount` is a **technical VAT-table field** (so we
leave it out and compare on the **tax-included** amount). The model maps cleanly onto
our target domain.

To validate V1 against **real** invoices, the remaining inputs, by priority:

1. **Structured line source (biggest lever):** can `GET /invoices/composed` (vs
   `/invoices/selected`) be the validated **read-only structured invoice-line
   source**? If yes, our engine consumes structured lines directly and **PDF parsing
   becomes a fallback** — a big reliability gain. One anonymized example response
   would let us confirm.
2. **2 anonymized invoice PDFs**, ideally two consecutive months for the same account
   with at least one visible change (e.g. an expired discount or out-of-bundle usage),
   plus the matching `bill-run-documents/search` metadata.
3. The **`type` / `code` / `vatType` catalogue** and how each maps to a business cause
   (discount expiry, overage, option change, proration, tax, one-off, adjustment).
4. The concrete flow to **list the two latest comparable invoices** for an account.
5. Minor: **tax rounding** rules, and whether the invoice total includes previous
   balance / payments or only current-period lines.

None of this blocks us — we're building on fixtures meanwhile — but your answers
unlock real-invoice validation. Happy to walk through it in a 30-min call.
