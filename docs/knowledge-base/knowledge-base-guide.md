...
> **Branch state (`feat/restart-from-scratch`, 2026-07-28, Sprint 9):** the KB
> **content** (`knowledge-base/*.md`) exists and the **runtime that ingests and
> answers from it is now built** on this branch — the Java backend runs the vector
> store (pgvector) and the RAG assistant. The "sync", `curl .../api/knowledge/...`
> and "ask the assistant" steps below work against the rebuilt backend on **port
> 8080** (some examples further down may still show the legacy `:8081` — use 8080).
