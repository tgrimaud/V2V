...
- Local full-stack run: `VOICE_BACKEND_URL` must be the **full** converse endpoint (`http://host:8080/api/conversation/converse`), not the base host — `conversation_backend/http_backend.py` derives the `converse-stream` and `warm-up` siblings from it. The WebRTC test page is served at `http://127.0.0.1:8090/webrtc.html` (`/` → `index.html`). The backend api-key gate is open when `CONVERSATION_API_KEY` is empty; source `GRADIUM_*` / `MISTRAL_*` from the repo-root `.env` (`set -a; . ../.env; set +a`).
